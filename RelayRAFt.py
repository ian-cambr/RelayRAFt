...
def convert_raw_files_core(source_folder, output_folder, 
        ...
        for root, dirs, files in os.walk(source_folder):
            for f in files:
                if f.lower().endswith(supported_input_extensions):
                    input_files_list.append((os.path.join(root, f), f))
    else:
        for f in os.listdir(source_folder):
            if f.lower().endswith(supported_input_extensions):
                input_files_list.append((os.path.join(source_folder, f), f))
    ...
    try:
        for index, (original_source_file_path, filename) in enumerate(input_files_list):
            ...
            is_png_file = filename.lower().endswith(".png")

            try:
                if is_raw_file:
                    if status_callback: status_callback(f"  Reading RAW: {filename}")
                    with rawpy.imread(original_source_file_path) as raw:
...
class RAFConverterApp:
    def __init__(self, root_window):
        ...
        self.root.title("Image Converter (cjxl / avifenc / exiftool)") # MODIFIED Title
        ...
        title_label = ttk.Label(main_frame, text="Image (RAF/PNG) to JXL/AVIF Converter", font=("Arial", 16, "bold")) # MODIFIED Label
        ...
        ttk.Label(folder_frame, text="Source Folder (RAF/PNG):").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W) # MODIFIED Label
        ...
        ttk.Checkbutton(folder_frame, text="Recursive", variable=self.recursive_var).grid(row=0, column=3, padx=5, pady=5)
    ...
    def browse_source_folder(self):
        ...
        folder_selected = filedialog.askdirectory(title="Select Source Image Folder (RAF/PNG)", initialdir=initial_dir) # MODIFIED Title
        if folder_selected:
    ...
    def start_conversion_thread(self):
        ...
        conv_thread = threading.Thread(target=convert_raw_files_core, args=(
            ...
            self.copy_metadata_var.get(), selected_format, self.recursive_var.get()), daemon=True)
        conv_thread.start()
